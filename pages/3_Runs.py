...
forecast_runs_directory = "./data/forecast/"

# Get forecast files from directory
forecast_files = [file for file in os.listdir(forecast_runs_directory) if file.endswith('.parquet')]

# Get creation dates of forecast files
forecast_files_with_dates = [(file, time.ctime(os.path.getctime(os.path.join(forecast_runs_directory, file)))) for file in forecast_files]

# Display dropdown list with creation dates
selected_file = st.selectbox('Select Forecast File', [f"{file} ({date})" for file, date in forecast_files_with_dates])

# Load data
if selected_file:
    selected_file = selected_file.split(" ")[0]  # Extract the file name from the selected option
    forecast = pd.read_parquet(os.path.join(forecast_runs_directory, selected_file))
else:
    data = None
